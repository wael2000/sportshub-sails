    ...
    setTimeout(function() {
      ...
      function goAheadAndActuallyConnect() {
        ...
        io.socket.on('connect', function socketConnected() {
		  ...
		  // add socket message
		  io.socket.on('invitee', inviteeReceived);
		  io.socket.on('eventinvitee', eventInviteeReceived);
		  //io.socket.on('event', eventInviteeReceived);
...
function inviteeReceived(message){
	var invitee = message.data;
	var row = '<td>' + invitee.salutation + '</td><td>' + invitee.name + '</td>';
				row += '<td>' + invitee.title + '</td><td>' + invitee.email + '</td>';
				row += '<td>' + invitee.mobile + '</td>';
	var tableRow = $("tr[id*=invitee"+message.id+"]");
	tableRow.html(row);
	tableRow.css("background-color", "yellow");
	setTimeout(function() {
    	tableRow.css("background-color", "white").effect("highlight", {}, 5000);
	}, 4000);
}


function eventInviteeReceived(message){
	console.log(message);
	if(message.verb == "updated"){
		var eventInvitee = message.previous;
		var data = message.data;
		if(eventInvitee)
			$.extend( eventInvitee, data );
		else 
			eventInvitee = data;
		if(eventInvitee.attendence == 1)
			attendeeJoined(eventInvitee);
	} else if(message.verb == "created"){
		//console.log(message);
		attendeeJoined(message.data)
	}
}
